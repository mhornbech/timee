//
//  RedmineSettingsViewController.m
//  Timee
//
...
#import "RedmineSettingsView.h"
#import "RedmineSettingsViewController.h"

@implementation RedmineSettingsViewController

@synthesize redmineSettingsView = _redmineSettingsView;
...
- (void)cancel
{
    [self.delegate redmineSettingsViewControllerDidFinish:self];
}
...
- (void)done
{
    ...
    UIAlertView *alertView = [Integration verifySettingsForApplication:@"redmine"];
    [self.redmineSettingsView.tableView setFrame:CGRectMake(0, 44, 320, [[UIScreen mainScreen] bounds].size.height - 44)];
    
    if (alertView == nil)
    {
        [self.redmineViewController refreshSettings];
...
- (NSString *)userDefaultsKeyForIndexPath:(NSIndexPath *)indexPath
{
    switch (indexPath.section) 
    {
        ...
        case 1:
            return indexPath.row == 0 ? @"redmineUsername" : @"redminePassword";
            break;
    }
...
- (void)viewDidLoad
{
    self.redmineSettingsView = (RedmineSettingsView *)self.view;
    
    if ([[UIScreen mainScreen] bounds].size.height == 568)
    {
        self.redmineSettingsView.frame = CGRectMake(0, 0, 320, 568);
        
        CGRect frame = self.redmineSettingsView.tableView.frame;
        self.redmineSettingsView.tableView.frame = CGRectMake(frame.origin.x, frame.origin.y, frame.size.width, frame.size.height + 88);
    }
...
{
    [self presentModalViewController:self.redmineViewController animated:YES];
}
...
{    
    ...
    NSIndexPath *indexPath = [self.redmineSettingsView.tableView indexPathForCell:cell];
    
    [self.redmineSettingsView.tableView setFrame:CGRectMake(0, 44, 320, [[UIScreen mainScreen] bounds].size.height - 44 - 216)];
    [self.redmineSettingsView.tableView scrollToRowAtIndexPath:indexPath atScrollPosition:UITableViewScrollPositionNone animated:YES];    
}
